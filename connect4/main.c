...
void print(char board[height][width]);
int checknum(int num, char board[height][width]);
void player(char board[height][width], int num, char character);
int checkfull(char board[height][width]);
int horizontalScore(char board[height][width], char character);
int verticalScore(char board[height][width], char character);
int diagonal(char board[height][width], char character);
int diagonal1(char board[height][width], char character);
int rowNum(int num, char board[height][width]);
int checkEmpty(char board[height][width]);
int checkCol(int num, char board[height][width]);
void undoRedo(char *x, char board[height][width], int num, int undoCol[height*width],
               int undoRow[height*width], int *k, int *l, int *z,
               int redoCol[height*width], int redoRow[height*width], int *q);
void Easy(char board[height][width], int *num);
int checkEmpty1(char board[height][width],int num,int numOfrow);
int Medium(char board[height][width], char character, int *num);
void Hard(char board[height][width], int *num);
void undoRedoLimit(int num, int *undoCounter, int *redoCounter);
int numX(char board[height][width]);
int numO(char board[height][width]);
void XMLformating();
void highscore(int high);
void saveLoad(int *num, char board[height][width]);
...
int main()
{
    ...
    char playAgain;
    do{
        int flag, score1=0, score2=0, compUndoCol[height*width], undoCounter=0,
            redoCounter=0, compUndoCounter =0, choose;
        int i, j, num=0, undoCol[height*width], k, undoRow[height*width],
            redoCol[height*width], redoRow[height*width], l, compUndoRow[height*width];
        static int q=0, z=0, d=0, c=0;
        char board[height][width], x='X', o='O';
        ...
        printf("Player1VsPlayer2 press(1)\n\nHumanVsComputer press(2)\n\n");
        scanf("%d", &choose);
        ...
        if(choose == 1){
            print(board);
            do{
                printf("\nif wanna undo press zero\n");
                printf("if wanna redo press -3\nLoad Press(-1)\nSAVE PRESS(-2)\n");
        ...
        else
        {
            ...
            do{
                printf("\nif wanna undo press zero\n");
                printf("if wanna redo press -3\nLoad Press(-1)\nSAVE PRESS(-2)\n");
                printf("\nUser enter number of the column:");
                scanf("%d", &num);
                system("CLS");
                saveLoad(&num, board);
                num = checknum(num,board);
        ...
        highscore(high);
        printf("\nif you wanna play again press (y)\nelse press any key\n");
        scanf("\n%c",&playAgain);
        system("CLS");
    }while(playAgain == 'y');
    return 0;
...
int checknum(int num, char board[height][width])
{
    ...
    {
        print(board);
        printf("\nPlease,Enter a number from 1 to %d :", width);
        scanf("%d", &num);
        system("CLS");
        }
    return num;
